...
def cvemap_search(cve, info_dir):
    vulnx_json_path = f"{info_dir}/cvemap.json"
    vulnx_command = f"vulnx id {cve} -j > {vulnx_json_path}"
    ...
    with open(vulnx_json_path) as f:
        json_data = json.load(f)

        if len(json_data) == 0:
            logging.error(f"Error in vulnx search: {cve}")
            return None
...
def count_cwe(cve_lst):
    ...
    for cve in cve_lst:
        ...
        with open(cvemap_json_path) as f:
            cvemap_json = json.load(f)
            if isinstance(cvemap_json, list):
                cvemap_json = cvemap_json[0] if cvemap_json else {}
...
def calculate_score(features, trending_score):
    ...
    if features["code"].get("GitHub") or features["code"].get("ExploitDB"):
        ...
        if has_code:
            ...
            trend_score_weighted_expdb = (
                trending_score_expdb * weights["trending_score"]
            )
            trend_score_weighted_github = (
                trending_score_github * weights["trending_score"]
            )

            # Handle cases where one source has no scores
            final_expdb = (
                (max_score_expdb + trend_score_weighted_expdb)
                if max_score_expdb > 0
                else 0
            )
            final_github = (
                (max_score_github + trend_score_weighted_github)
                if max_score_github > 0
                else 0
            )
    ...
    if code_results and doc_results:
        ...
        doc_scores, doc_max_repo, doc_max_score, doc_final_score, doc_has_code = (
            doc_results
        )
...
def calculate_match_stats(values1, values2):
    ...
    near_matches = sum(
        abs(["easy", "medium", "hard"].index(v1) - ["easy", "medium", "hard"].index(v2))
        == 1
...
def create_df(cvss_data, epss_data, pentestasst_data):
    all_keys = sorted(
        set(cvss_data.keys()).union(epss_data.keys(), pentestasst_data.keys())
    )
    data = {
...
def num_agreement_analysis(df):
    ...
    for col1, col2 in combinations(df.columns, 2):
        ...
        print(
            f"Rows where the difference between {col1} and {col2} is larger than {threshold}:"
        )
...
def visualize_num_results(df, output_dir="plots/"):
    ...
    for col1, col2 in combinations(df.columns, 2):
        ...
        plt.axhline(
            np.mean(diff_values), color="red", linestyle="--", label="Mean Difference"
        )
        plt.axhline(
...
def visualize_bin_results(cvss_data, epss_data, pentestasst_data, output_dir):
    ...
    for (name1, data1), (name2, data2) in combinations(
        ...
        confusion = confusion_matrix(
            list(data1.values()), list(data2.values()), labels=labels
        )
    ...
    data = {
        ...
        "EEAS": [
            list(pentestasst_data.values()).count(c) for c in ["easy", "medium", "hard"]
        ],
    }
...
def cve_classifier(cve, output_dir="resources/", mode="specific"):
    ...
    if mode == "specific":
        ...
        epss_score = cvemap_json.get(
            "epss_score", cvemap_json.get("epss", {}).get("epss_score", 0)
        )
        epss_percentile = cvemap_json.get(
            "epss_percentile", cvemap_json.get("epss", {}).get("epss_percentile", 0)
        )
    elif mode == "general":
        cvss_score = 0
        epss_score = 0
...
def product_to_cve(product, output_dir):
    ...
    for p in cleaned_product:
        ...
        vulnx_json_path = f"{output_dir}/{product_dir_name}/vulnx_{p_dir_name}.json"
        vulnx_command = (
            f"vulnx search 'affected_products.product:\"{p}\"' -j > {vulnx_json_path}"
        )
        subprocess.run(
            vulnx_command,
            shell=True,
...
def analyze_cve_lst(cve_lst, output_dir, app_name):
    ...
    normalized_has_code_num_df.to_csv(
        f"{output_dir}/normalized_has_code_score_results.csv"
    )
    ...
    avg_searching_time = total_searching_time / (
        (len(cve_lst) + 1) if app_name else (len(cve_lst))
    )
    avg_analysis_time = total_analysis_time / (
        (len(cve_lst) + 1) if app_name else (len(cve_lst))
    )
    logging.info(
...
def get_exp_info(cve_lst=[], output_dir="", app_name=""):
    ...
    try:
        ...
        Settings.llm = OpenAI(
            temperature=cve_config["temperature"], model=cve_config["model"]
        )
    ...
    total_searching_time, total_analysis_time = analyze_cve_lst(
        cve_lst, output_dir, app_name
    )
