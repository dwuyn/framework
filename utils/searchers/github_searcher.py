...
class GithubSearcher:
    ...
    def search_keyword_metadata(self, keyword: str, limit: int = 12, loose_mode: bool = False):
        mode = True if loose_mode else False
        ...
        if not repo_result:
            return []
        items = repo_result.get('items', [])
        return items[:limit]

    def clone_candidate(self, item: dict, output_dir: str) -> str:
        os.makedirs(output_dir, exist_ok=True)
        repo_name = str(item.get('name') or 'repo')
        language = str(item.get('language') or 'unknown')
        stars = int(item.get('stars_count') or item.get('stargazers_count') or 0)
        repo_url = str(item.get('clone_url') or '')
        repo_directory = os.path.join(output_dir, f'{stars}_{repo_name}_{language}')
        ...
        try:
            ...
            pass
        Path(repo_directory).mkdir(parents=True, exist_ok=True)
        return repo_directory
