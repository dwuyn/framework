...
class GoogleSearcher:
    ...
    def search_keyword_metadata(self, keyword: str, limit: int = 3):
        try:
    ...
    def materialize_links(self, links, output_dir: str):
        self.create_directories(output_dir, [
            (
                (item.get("title") or item.get("link", "doc")).split("//")[-1].replace("/", "-")[:30],
                item.get("link", ""),
            )
            for item in links
            if item.get("link")
        ...
        for root, _, files in os.walk(output_dir):
            for file_name in files:
                if file_name == "R_DOC.md":
                    path = os.path.join(root, file_name)
                    try:
                        with open(path, encoding="utf-8", errors="ignore") as handle:
                            link = handle.readline().replace("link to this page is ", "").strip()
                    except Exception:
                        link = ""
                    manifests.append({
                        "path": path,
