...
WORKDIR /opt/build
COPY envelope/requirements.cpu.lock /opt/build/requirements.lock
...
LABEL org.opencontainers.image.title="HackSynth" \
      com.veriplanpt.framework="HackSynth" \
      com.veriplanpt.runtime="cpu-only" \
      com.veriplanpt.upstream-commit="$UPSTREAM_COMMIT" \
...
COPY source/ /opt/upstream/
COPY adapter/ /opt/adapter/
ENV PATH="/opt/venv/bin:$PATH" PYTHONPATH="/opt/upstream" VERIPLANPT_SOURCE_DIR="/opt/upstream" VERIPLANPT_RUN_DIR="/run/veriplanpt" CUDA_VISIBLE_DEVICES=""
RUN addgroup --gid 10001 baseline \
    && adduser --disabled-password --gecos "" --uid 10001 --gid 10001 baseline \
    && mkdir -p /run/veriplanpt \
    && chown -R baseline:baseline /run/veriplanpt /opt/upstream /opt/adapter \
