...
class OpenAIChat(ABC):
    def __init__(self, config):
        self.config = config
        # Support custom headers for vLLM/ngrok endpoints
        default_headers = getattr(config, 'default_headers', None)
        if default_headers is None and 'ngrok' in str(config.base_url):
    ...
    def chat(self, history: List) -> str:
        try:
            logger.info(f"Sending request to {self.config.base_url} with model {self.model_name}")
            ...
            logger.info(f"LLM response received: {ans[:200] if ans else 'None'}...")
            return ans
