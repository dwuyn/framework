...
class WritePlan(BaseModel):
    ...
    def run(self, init_description) -> str:
        ...
        if rsp is None or rsp == "":
            raise ValueError("LLM returned empty response. Check model connection.")
        ...
        # Try to find JSON array directly if no <json> tags
        json_match = re.search(r'\[[\s\S]*\]', rsp)
        if json_match:
            return json_match.group(0)
        
        raise ValueError(f"LLM response did not contain valid JSON. Response: {rsp[:500]}")
